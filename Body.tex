...
\begin{lstlisting}
   npm install
\end{lstlisting}
In the Neolexon case there where no problems with the above setup and version durin the installation. If so the package manager (npm) will
...
\end{figure}
In case you dont use the standard local mongoDB databse you need to specify the port of the mongoDB you are using by
\begin{lstlisting}
...
\end{lstlisting}
\cite{https://github.com/Strider-CD/strider}
...
% later
Note that if there are errors during the installation of plugins, the reason can be that you are not logged in ans an administrator,
...
To run and use strider as it was intendet there are several configuration steps that are needed to be done. We recommend implementing
an upstart job that handles start, stop and status evaluation for the strider service. The following code is used in the neolexon
prject and was taken from \url{https://fosterelli.co/creating-a-private-ci-with-strider.html}
...
In case the manuall testing is running correct the next step is to enable strider to run tests automatically after a new push to the
...
% where is the hook up process?? was done via UI!!!
