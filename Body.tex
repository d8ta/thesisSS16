...
between failure and success. Webapplications and services needs to change and scale over time. Sometimes a service gets customers or is
growing fast. Runtastic, a small company from Linz/austria which developed a fitness application wheere having a tremendes growth of
their userbase, in a short periode of time. Most of the time users demands a 24/7 availability of the service they use a goode example
...
got a huge ammount of interest from local, national and international media (\url{http://www.bbc.com/news/world-us-canada-34383655} and
from their usersbase.
...
(\cite{lindstrom2004extreme}). There is alread a reliable base of knowledge and literature for Continous Integration in todays webdeveloper community
...
The Question this thesis will research is: What is a best practise apporoach for modern webapplication,
to provide automatization and continously integration features. It is the goal of this thesis to show how to establish a modern development operation
cycle for webapplications using Node.JS and the MEAN Stack.
This thesis will citeing the standard literature and methods for continuous integration and deployment. After this this thesis will
show how to build a MEAN Stack application in a safe and stable way.
For this the thesis will research the literatur available for continuous integration and autmatization, to build a similar system with the MEAN Stack.
To do so the thesis will describe how to automatize builds and tests. The description will be based on the implementation cycle of the neolexon webapplication.
...
In civil engineering the construction phase uses the majority of the time, money and the ressources given for the project.
In his essay (\cite{fowler2001new}) Martin Fowler writes that only 10 percent of the available ressources in civil engineering are used for
...
brought software development into trouble, because seperating design and construction, common in other fields of engineering, like building
a bridge does not fit the needs of software engineering.

In civil engineering a very detailed plan is made, and after a careful mathematical analysis it is descided to put this plan into practice. For constructing,
...
intellecutal environment. The construction ressources in general dont need to understand the whole design and therefore can concentrate on
concentrate about the construction. The trouble for software development with seperation of design and construction is, that the whole process of programming
, according to fowler, is desiging in the first place. \cite{opac-b1105529} stats that only 15 percent of a software project is code.
This stands in contrast to the building example from civil engineering in which the most percentage of time and ressources going into construction.
...
detail to the customer. In software engineering and webdevelopment there is proven method to mathematically analyse a software design. This means that there is
...
consider the following neolexon example application. The webapplication should provide information for speach therapists in a simple and understandable dashboard,
in which the user (therapist) gets an overview about what is going on. The descision was made that the dashboard should hold only the list of patients and wordlists (figure 1).
...
After the dashboard was created and the feedback of several therapists arrived, the descision was made that the dashboard did not need to hold the lists of words, because all lists where individual to the patients.
after this descision, was left with only the patientlist. In case of the application this was not the main goal for the invented dashboard. To conquer this the development team came up with another
...
Now you have a sidenavigation with patients and the statistics for each of them. This transformation would have been very
complicated to achieve with traditional engineering methaphore, because the clients only would have seen it at the end of the process. To rebuild vital
parts of a webapplication is a complicated process. The changes occured in a timeframe of 2 weeks, from plan to final idea.
This example shows that traditioonal engineering strategies are not working well in terms of webdevelompent and software.
...
invented an idea of how to handle softwareproject. On the 13. of february 2001 they created \textbf{\textit{a Manifesto for Agile Software Development}}.
...
Based on the manifesto principles it is more realistic to build software, because the princilples take the different approach of design and construction into
...
without hange during the process, agile development takes into account that most sofware is changeing over the course of the
...
But satisfing customer needs is only one part about this approach. Another reason to continuosly integrate software is about the change and the mantainability.
...
This Thesis assumes that we setup up Strider to deploy on DigitalOcean. DigitalOcean offers a viritual private server (VPS) for 5 euro per month. This
...
strider.conf file. \textbf{HOST}, \textbf{USER} and \textbf{PASS} are the main information. In this case the host is mailgun. The user and pass are your userdate
...
project and that is having the abiltiy to fail. If digital ocean is experiencing downtime during the push of new developed code, the deployement
...
Concentrate on the necessary software and third partys and try to avoid dependencies that can cause problems without a certain value for the project
or the customer. A working continuous delivery cylce is only as good as the uptime of the different and single components that represent the CI service.

Choose the software that is useful and reliable and that fits the needs of the project. In case of the neolexon CI the descision for striderCD came from
multiple sources of information. Since there is no reserach literatur about strider the neolexon development and dev ops team uses stackoverflow and the
...
SPOF cannot be fully avoided in todays webdevelopment and application managment. In \cite{ulbrich2012eliminating} another solution to avoid SPOF is
the use of redundant systems or similar systems and software. Reduncancy means that in case of failure another system took over. A useful example
can be another CI service besides strider that took over if strider suffers from downtime. An automatisation like this can be useful for bigger
projects that will suffer heavy consequences in case of a failure. For reasons of symplicity and complexity the neolexon project uses no redundant
