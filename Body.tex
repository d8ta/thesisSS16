...
Continous integration and delivery describes the method of shipping and developing software in short cyles. CI covers an important part of the agile software
development movement. The main idea is to ensure that the application or software stays functional at any level. In Continous Integration the development
process runs in short cylcles that produce a certain feature or element of the project. If this small piece of functionality proves to be error free and
tested the developer integrates it into the existing project. One of the advantages of this technique is that every time a feature or piece
of code is added, advanced or altered the developer stays in controll. If everything was working before, the problem is in that feature or code that was
integrated right now. On the other hand, if a team uses long development cycles it may be impossible to know what parts of the program cerates an error.
...
part of an application, for example if the email validation is working correctly) combined with continously integrate and deliver software provides a
good setup for creating software.
...
% start here
\subsection{The role of development operations}
Today building a web application contains several parts and ideas that needs to be combined. Besides the layout and look of the frontend applications
...
frontend, that can be used today to ship an application. For this thesis i will describe using NodeJS and the MEAN stack to create and deliver an
applications with methods of continous integration. For this task development operations are needed. If a developer needs to ship and test code and than
publish it, development operations will provide the needed operations and background to test and ship the code from a local setup to any kind of
server. To do so development operations seek to automatice the process of development and shipping and to make sure that new features or code is
tested before integrated into a running system. This amount of automatication is using different services and setups to test, integrate and deliver
modern web applications. For this a continous deployment pipline is needed (figure 4)

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth]{images/pipeline.png}
  \caption{development operation with continous operations}
\end{figure}
